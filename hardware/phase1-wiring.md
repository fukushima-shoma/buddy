...
会話開始用の押しボタンは、モーターと距離センサーが使用していないGPIO17を使う。
ボタンに極性はない。Raspberry Piの電源を切ってから、次の2本を接続する。

| 押しボタン | Raspberry Pi | 役割 |
| --- | --- | --- |
| 片側 | GPIO17・物理ピン11 | 会話開始入力 |
| 反対側 | GND・物理ピン9など | 押下時にLOWへする |

```text
物理ピン11（GPIO17） ── 押しボタン ── GND（物理ピン9など）
```

コード側で内部プルアップを有効にするため、外付け抵抗や3.3Vへの配線は不要。
GPIO17はBCM番号であり、物理ピン17ではない。ボタンへ5Vを接続しない。
