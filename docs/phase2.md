...
人物を見失った場合、距離が未取得で人物が中央にいる場合、60cm以下に障害物がある
場合は停止判断になる。距離が未取得でも人物が左右にいる場合は、前進せず短い旋回
だけを許可して距離センサーを人物へ向け直す。70cm以上へ離れるまで障害物停止を
保持する。この段階ではモーター用電池をOFFのままにする。

人物は直近3フレーム中2回、近い水平位置で確認してから追跡判断を開始し、追跡中の
1フレームだけの欠落を許容する。左右位置と距離には直近3回の中央値を使い、単発の誤検出や距離スパイクを
抑える。60cm以下の生距離では即座に停止し、70cm以上を5フレーム連続確認するまで
...
人物が中央なら前進、左右ならパルス旋回する。人物の確認中・消失、人物が中央での
距離未取得、障害物検出では停止する。距離未取得でも人物が左右なら、前進せず
パルス旋回だけを行う。すべて確認するまで車輪を床へ下ろさない。
