...
`--proactive-interval 900`を指定すると、8時から20時の間に15分間呼びかけがない場合、
Buddyから「ねえ、いっしょにお話ししない？」と一度だけ誘って会話を開始する。
応答がなければ1回の無音判定で待機へ戻る。開始・終了時刻は
`--proactive-start-hour`と`--proactive-end-hour`で変更でき、間隔を0にすると無効になる。
